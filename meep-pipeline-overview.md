...
- **Monitor z-span touches PML boundary:** `get_port_z_span()` returns the full layer stack z-range, which is used as the mode monitor z-size. After `set_z_crop()`, the layer stack z-extent exactly matches the PML inner boundary (e.g. z=-0.50 to z=0.72 with dpml=1.0). At coarse resolution, the outermost monitor pixels may straddle the PML interface, corrupting the eigenmode overlap integral at the edges. Fix: shrink the monitor z-span by a small margin (e.g. `z_span - 2/resolution`) so its edges sit clearly inside the non-PML region. The guided mode evanescent tails at the z-edges are ~3% of peak (for 220nm SOI), so a 1-pixel inset has negligible impact on mode overlap accuracy.
- **Empty `core2` layer in config:** When using ubcpdk's ebeam_y_1550, the layer stack includes a `core2` entry (GDS layer [31, 0] = WG2) with the same z-range and material as `core`. This component has zero geometry on layer 31, so the runner extracts 0 polygons and creates 0 prisms for it — harmless but adds clutter to the config. Consider filtering out layer stack entries that produce no polygons, or letting the user exclude specific layers via `set_stack()`.
