...
def sample_topview_field(
    ...
    *,
    ...
    sampled = probe.sample(source, snap_to_closest_point=snap_to_closest_point)
    ...
    if "vtkValidPointMask" in sampled.point_data:
        ...
        if not valid.any() and not snap_to_closest_point:
            sampled = probe.sample(source, snap_to_closest_point=True)
            arr = sampled.point_data[field]
            if arr.ndim == 2:
                values = (
...
def plot_cross_section(
    ...
    *,
    ...
    sampled = probe_grid.sample(sliced)
