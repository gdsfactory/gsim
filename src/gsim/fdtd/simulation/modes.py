"""Tidy3D mode solver module.

Tidy3D has a powerful open source mode solver that can:
- Compute bend modes
...
from gsim.fdtd.materials import MaterialSpecTidy3d, get_medium
...
class Waveguide(BaseModel, extra="forbid"):
    ...
    Parameters:
        ...
        core_material: core material specification.
        ...
        sidewall_thickness: thickness of a layer on the sides of the waveguide core.
        sidewall_k: absorption coefficient added to the core material on the side-surface.
        surface_thickness: thickness of a layer on the top of the waveguide core.
        surface_k: absorption coefficient added to the core material on the top-surface.
        bend_radius: radius to simulate circular bend.
        num_modes: number of modes to compute.
        group_index_step: calculate group index if True or positive float.
    ...
    def _fix_wavelength_type(cls, v: Any) -> np.ndarray:
    ...
    @property
    def n_group(self):
        """Group index (only present if group_index_step is set)."""
    ...
    def loss_dB_per_cm(self):
        ...
        wavelength = self.wavelength * 1e-6
        alpha = 2 * np.pi * np.imag(self.n_eff).T / wavelength
        return 20 * np.log10(np.e) * alpha.T * 1e-2
    ...
    def plot_field(
        ...
        **kwargs,
        ...
        Parameters:
            ...
            value: component of the field ('real', 'imag', 'abs', 'phase', 'dB').
...
class WaveguideCoupler(Waveguide):
    ...
    Parameters:
        core_width: width of each core (tuple of two values).
